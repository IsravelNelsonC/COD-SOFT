...
class ContactBookApp:
    ...
    def create_widgets(self):
        ...
        tk.Label(header_frame, text="Contact Book", font=('Arial', 24, 'bold'), 
                bg=self.colors['header'], fg='white').pack(pady=20)
        ...
        tk.Label(list_frame, text="Contacts", font=('Arial', 14, 'bold'), 
                bg=self.colors['list_bg']).pack(pady=10)
        ...
        search_btn = tk.Button(search_frame, text="Search", bg=self.colors['button'], fg='white',
                             font=('Arial', 10, 'bold'), command=self.search_contact)
        ...
        for i, label in enumerate(labels):
            tk.Label(form_frame, text=label, bg='#f0f8ff', font=('Arial', 11)).grid(row=i, column=0, sticky='e', pady=5, padx=5)
            var = tk.StringVar()
